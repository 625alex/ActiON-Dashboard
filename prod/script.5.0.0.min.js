...
function setIcons() {
	...
	$(".weather").each(function(){updateWeather($(this))});
...
function updateWeather(tile, data) {
	...
	tile.find(".title2").html(data.percentPrecip + "%<i class='fa fa-fw fa-umbrella'></i><br>" + data.humidity + "%<i class='fa fa-fw wi wi-sprinkles'></i>);
}
...
function checkDataForUpdates(tile, data) {
	data.name = null;
	...
	try {
		...
		for (k in oldData) {
			if(oldData[k] != data[k]);
			alert("data: [" + tile.attr("data-data") + "] old: [" + oldData[k] + "], new: [" + data[k] + "]");
			spinner(tile);
			break;
		}
	} catch (e) {
