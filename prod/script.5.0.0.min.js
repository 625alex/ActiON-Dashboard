...
function setIcons() {
	...
	$(".thermostatHeat").each(function(){renderThermostat($(this))});
	$(".thermostatCool").each(function(){renderThermostat($(this))});
...
function renderThermostat(tile) {
	var data = JSON.parse(tile.attr("data-data"));
	tile.empty();
	var content = "<div class='title'>" + tile.attr("data-name") + "<br/><span class='title2'>" + data.temperature + "&deg;, " + data.thermostatOperatingState + "</span></div>\n\
<div class='icon'>" + data.setpoint + "&deg;</div>\n\
<div class='icon up'><i class='fa fa-fw fa-chevron-up'></i></div><div class='icon down'><i class='fa fa-fw fa-chevron-down'></i></div>\n\
<div class='footer'>&#10044; " + data.thermostatFanMode + (data.humidity ? ",<i class='fa fa-fw wi wi-sprinkles'></i>" + data.humidity  + "%" : "") + "</div>";
	tile.html(content);
}
...
function updateTile(data) {
	if (data.tile == "device") {
		...
		if (data.type == "thermostatHeat" || data.type == "thermostatCool") {
			tile.attr("data-data", JSON.stringify(data);
		} else if (data.tile == "weather") {
