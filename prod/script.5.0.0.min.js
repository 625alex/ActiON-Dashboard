...
function updateThermostat(tile) {
...
function updateTile(data) {
	if (data.tile == "device") {
		...
		if (data.type == "thermostatHeat" || data.type == "thermostatCool") {
			var d = {humidity: data.humidity, temperature: data.temperature, thermostatFanMode: data.thermostatFanMode, thermostatOperatingState: data.thermostatOperatingState, setpoint: data.type == "thermostatHeat" ? data.heatingSetpoint : data.coolingSetpoint};
			var oldData = JSON.parse(tile.attr("data-data"));
			for (k in oldData) {
				if(oldData[k] != d[k];
				spinner(tile);
				break;
			...
			updateThermostat(tile);
		} else if (data.tile == "weather") {
			data.name = null;
			
			var oldData = JSON.parse(tile.attr("data-weather"));
			for (k in oldData) {
				if(oldData[k] != data[k];
				spinner(tile);
				break;
			...
			tile.attr("data-weather", JSON.stringify(data));
			renderWeather(tile);
		} else {
